...
if anschlussleistung_kw > 0 and verbrauch_kwh > 0:
    ...
    st.write(f"Energiekosten:                {energiekosten:10.2f} €")
    st.write(f"Grundpreis:                   {grundkosten:10.2f} €")
    st.write(f"Messleistungskosten:          {messleistungskosten:10.2f} €")
    st.write(f"Wärmezählerkosten:            {waermezaehler_kosten:10.2f} €")
    st.markdown("---")
    st.write(f"Öko- und Umweltbeitrag:       {oekobeitrag:10.2f} €")
    st.write(f"Benutzungsabgabe (6%):        {benutzungsabgabe:10.2f} €")
    st.markdown("---")
    st.write(f"**Gesamtkosten netto:**        {netto_gesamt:10.2f} €")
    st.write(f"Mehrwertsteuer (20%):         {mehrwertsteuer:10.2f} €")
    st.markdown("---")
    st.success(f"**Gesamtkosten (brutto):       {brutto_gesamt:10.2f} € / Jahr**")
